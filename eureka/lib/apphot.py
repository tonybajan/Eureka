...
def apphot(iii, meta, image,  ctr, photap,  skyin, skyout, betahw, targpos,
  ...
  skyann  = np.bitwise_xor(di.disk(iskyout, ictr, isz), di.disk(iskyin,  ictr, isz))
  ...
  if med: # Do median sky
    ...
    if imerr is not None:
      ...
      dummy, ret[skyerr]  = me.meanerr(iimage, iimerr, mask=skymask, err=True)
  ...
  if isbeta == True and betahw > 0:
      ...
      betabox = image[ctr_y-betahw:ctr_y+betahw+1,ctr_x-betahw:ctr_x+betahw+1]
      ...
      raise ValueError("Could not evalaute beta. Please update POET photom.pcf to betahw > 0.")
  ...
  ax.imshow(image, origin='lower', vmax = 500)
  ax.scatter(targpos[1], targpos[0], marker='x', s=25, c='r')
  alphas = np.zeros(image.shape)
  alphas[np.where(skyann == True)] = 1
  ax.imshow(~skyann, origin='lower', alpha=alphas)
  alphas = np.zeros(image.shape)
  alphas[np.where(apmask == True)] = 1
  ax.imshow(~apmask, origin='lower', alpha=alphas)
  plt.tight_layout()
  plt.savefig(meta.outputdir + '/figs/frame_new_{0}.png'.format(iii))
